...
RSpec.describe 'Minutes::Topics API', type: :request do
  ...
  context 'when update topic' do
    it 'update topic successfully' do
      expect do
        patch api_minute_topic_path(minute, topic), params: { topic: { content: '話題にしたいこと・心配事を更新しました' } }
      end.to change { Topic.find(topic.id).content }.from('gitのブランチ履歴がおかしくなってしまったので、どなたかペアプロをお願いしたいです').to('話題にしたいこと・心配事を更新しました')
    end
  ...
  context 'when destroy topic' do
    it 'destroy topic successfully' do
      expect do
