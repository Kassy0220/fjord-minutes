...
RSpec.describe MeetingSecretary, type: :model do
  describe '.prepare_for_meeting' do
    it 'create minute and notify today meeting for all course', skip: 'this class is removed later' do
  ...
  describe '#create_minute' do
    ...
    it 'create minute of next meeting', skip: 'this class is removed later' do
    ...
    it 'does not create minute if latest meeting is not finished', skip: 'this class is removed later' do
  ...
  describe '#create_first_minute' do
    ...
    it 'does not create multiple minutes with the same meeting date', skip: 'this class is removed later' do
      travel_to latest_meeting_date + 1.day do
        expect { meeting_secretary.create_first_minute }.to change(rails_course.minutes, :count).by(1)
  ...
  describe '#notify_today_meeting' do
    ...
    it 'send discord notification', skip: 'this class is removed later' do
      travel_to latest_minute.meeting_date do
    ...
    it 'does not send notification when there is not minute', skip: 'this class is removed later' do
      meeting_secretary.notify_today_meeting
      expect(Discord::Notifier).not_to have_received(:message)
    end

    it 'does not send notification when today is not meeting date', skip: 'this class is removed later' do
      travel_to latest_minute.meeting_date - 1.day do
        meeting_secretary.notify_today_meeting
        expect(Discord::Notifier).not_to have_received(:message)
      end
    ...
    it 'does not send notification when notification is already sent', skip: 'this class is removed later' do
      travel_to latest_minute.meeting_date do
        meeting_secretary.notify_today_meeting
        meeting_secretary.notify_today_meeting
        expect(Discord::Notifier).to have_received(:message).once
